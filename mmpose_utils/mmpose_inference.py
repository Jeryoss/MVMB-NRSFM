...
def main():
    ...
    output_file_name = call_args['inputs'].replace("mp4", "npy")
    if diaplay_alias:
        ...
        for tmp in inferencer(**call_args):
            for p in range(len(tmp["predictions"][0])):
                landmarks_data.append([
                    {"bbox": tmp["predictions"][0][p]["bbox"],
                     "landmarks": [[points[0], points[1], c] for points, c in zip(tmp["predictions"][0][p]["keypoints"],
                                                                                  tmp["predictions"][0][p][
                                                                                      "keypoint_scores"])]
                     }])
        end_time = time.time()
        print("Processing time: ", end_time - start_time)
        print("Frame count:", len(landmarks_data))
        print("FPS: ", len(landmarks_data) / (end_time - start_time))

        np.save(output_file_name, landmarks_data)
