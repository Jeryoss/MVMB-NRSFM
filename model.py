...
class Nrsfm_Net(nn.Module):
    def __init__(self, cfg):
        ...
        for w in self.weights:
            enc = nn.ConvTranspose2d(in_channels=w.shape[0], out_channels=w.shape[1], kernel_size=(w.shape[2], w.shape[3]), stride=1, padding=0)
            enc.weight = w
        ...
        self.coef_layer = nn.Conv2d(in_channels=cfg.num_atoms_bottleneck, out_channels=cfg.num_atoms_bottleneck, kernel_size=(cfg.BLOCK_HEIGHT, cfg.BLOCK_WIDTH), stride=1, padding='valid', bias=True)
        self.camera_layer = nn.Conv2d(in_channels=cfg.num_atoms_bottleneck, out_channels=1, kernel_size=1, stride=1, padding='valid', bias=True)
...
class Multi_RF_Nrsfm_Net(Nrsfm_Net):
    ...
    def forward_rf(self, x):
        ...
        return x[:, :camera_atoms].view(b, self.cfg.BLOCK_HEIGHT, self.cfg.BLOCK_WIDTH), x[:, camera_atoms:].view(b, self.cfg.num_atoms_bottleneck, 1, 1)
...
class Siamese_RF_Nrsfm_Net(Multi_RF_Nrsfm_Net):
    ...
    def forward_rf(self, x_a, x_b):
        ...
        cam = x[:, self.cfg.num_atoms_bottleneck:self.cfg.num_atoms_bottleneck+camera_atoms].view(bs, self.cfg.BLOCK_HEIGHT, self.cfg.BLOCK_WIDTH)
